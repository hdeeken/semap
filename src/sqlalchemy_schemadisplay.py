...
def _mk_label(
        mapper, show_operations, show_attributes, show_datatypes, bordersize):
    html = '<<TABLE CELLSPACING="0" CELLPADDING="1" BORDER="0" '\
        'CELLBORDER="%d" BALIGN="LEFT"><TR><TD>'\
        '<FONT POINT-SIZE="10">%s</FONT></TD></TR>' % (
            ...
            '<BR ALIGN="LEFT"/>'.join(
                '%s(%s)' % (
                    ...
                    ", ".join(
                        ...
                        and ("%s") % arg or ("%s=%s" % (arg, repr(default)))
                        for default, arg in
                        zip(
                            (func.func_defaults and
                             len(func.func_code.co_varnames) - 1 -
                             (len(func.func_defaults) or 0)
                                or func.func_code.co_argcount - 1) *
                            [_mk_label] + list(func.func_defaults or []),
...
from sqlalchemy.dialects import postgresql as PSQLdialect
...
def _render_table_html(table, metadata, show_indexes, show_datatypes):
    ...
    def format_col_str(col):
        if show_datatypes:
            ...
            return "- %s" % col.name
    html = '<<TABLE BORDER="1" CELLBORDER="0" CELLSPACING="0"><TR>'\
        '<TD ALIGN="CENTER">%s</TD></TR><TR>'\
        '<TD BORDER="1" CELLPADDING="0"></TD></TR>' % table.name

    html += ''.join('<TR><TD ALIGN="LEFT" PORT="%s">%s</TD></TR>' %
                    (col.name, format_col_str(col)) for col in table.columns)
    if metadata.bind and isinstance(metadata.bind.dialect, PSQLdialect):
        # postgres engine doesn't reflect indexes
        indexes = dict(
            (name, defin) for name, defin in metadata.bind.execute(text(
...
def create_schema_graph(
        tables=None, metadata=None, show_indexes=True,
    ...
    graph = pydot.Dot(prog="dot", mode="ipsep", overlap="ipsep",
                      sep="0.01", concentrate=str(concentrate),
                      rankdir=rankdir)
    for table in tables:
        graph.add_node(pydot.Node(
            ...
            label=_render_table_html(
                table, metadata, show_indexes, show_datatypes),
    ...
    for table in tables:
        for fk in table.foreign_keys:
            ...
            graph_edge = pydot.Edge(
                ...
                arrowtail=(
                    fk.parent.primary_key or fk.parent.unique)
                and 'empty' or 'crow',
                fontname=font,
